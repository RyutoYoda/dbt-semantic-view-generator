...
- **自動分類**: GPT-4がカラムをFACTまたはDIMENSIONに自動分類
- **YML連携**: 既存のdbtモデルドキュメントを活用
- **バージョン管理**: セマンティックビューを自動的にバージョン管理（v1, v2, v3...）
- **GitHub Actions対応**: マージ時の自動生成CI/CDパイプライン
...
### 問題: 間違った分類（FACT vs DIMENSION）

**解決策:**
1. YML説明を追加してカラムの目的を明確にする
2. より説明的なカラム名を使用
3. 生成されたビューを手動で編集
...
**解決策:**
1. カラムが最終的なSELECT文にあるか確認
2. カラム名に特殊文字が含まれていないか確認
3. SQLが有効で解析可能か確認
